    ...
    internal static class ReplaykitSettings
    {
        ...
        public static JObject GetDefaultSettings()
        {
            return new JObject
            {
                ...
                ["clipDir"] = "",
                ["clipKeybind"] = new JObject { ["key"] = "OBS_KEY_F8" },
                ...
                // where the user dragged each overlay, as canvas fractions of its visible top-left, keyed by style. empty = the canonical corner the transforms compute.
                ["overlayPosition"] = new JObject(),
                ...
                ["shareMode"] = "projector",
                ["discord_screenshare_enabled"] = false,
                ...
                // armed by the installer on a genuine first install; the dock opens the setup wizard on the next obs launch and /setup/complete clears it.
                ["setupWizardPending"] = false,
            };
        }
        ...
        private static JObject NormalizeClipKeybind(JToken value) =>
            NormalizeHotkeyCombo(value, new JObject { ["key"] = "OBS_KEY_F8" }, "clip keybind");
        ...
        private static JObject NormalizeOverlayPosition(JToken value)
        {
            var result = new JObject();
            if (!(value is JObject map)) return result;
            foreach (var style in new[] { "input_overlay", "bongo_cat" })
            {
                if (!(map[style] is JObject at)) continue;
                var xt = at["x"];
                var yt = at["y"];
                if (xt == null || yt == null || xt.Type == JTokenType.Null || yt.Type == JTokenType.Null) continue;
                double x, y;
                try { x = xt.Value<double>(); y = yt.Value<double>(); }
                catch (Exception ex) when (ex is FormatException || ex is InvalidCastException || ex is ArgumentException) { continue; }
                // same window the drag itself is clamped to -- a sliver may hang off any edge, nothing may be parked far outside
                if (double.IsNaN(x) || double.IsNaN(y) || double.IsInfinity(x) || double.IsInfinity(y)) continue;
                if (x < -1.0 || x > 2.0 || y < -1.0 || y > 2.0) continue;
                result[style] = new JObject { ["x"] = x, ["y"] = y };
            }
            return result;
        }
        ...
        public static JObject Normalize(JObject raw)
        {
            ...
            return new JObject
            {
                ...
                ["overlayPosition"] = NormalizeOverlayPosition(data["overlayPosition"]),
                ...
                ["setupWizardPending"] = GetBoolSetting(data, "setupWizardPending", defaults["setupWizardPending"].Value<bool>()),
            };
        }

        // whether the first-run setup wizard still needs to be shown. fail-closed: any read error -> not pending, so a
        // broken settings file never traps the user in a wizard loop.
        public static JObject GetSetupState()
        {
            bool pending = false;
            try { pending = ReadSettings()["setupWizardPending"]?.Value<bool>() ?? false; }
            catch (Exception ex) { Log.Write("GetSetupState: " + ex.Message); }
            bool deferred, alreadyOpened;
            lock (Server.State.UpdateCheckLock)
            {
                deferred = Server.State.StartupUpdatePending;
                alreadyOpened = Server.State.SetupWizardOpened;
            }
            if (pending && alreadyOpened)
                return new JObject { ["ok"] = true, ["pending"] = false, ["alreadyOpened"] = true };
            // an update wins the race: report not-pending so nothing opens the wizard now, and leave the flag set in settings so the wizard opens on the next launch once the install has landed
            if (pending && deferred)
                return new JObject { ["ok"] = true, ["pending"] = false, ["deferredForUpdate"] = true };
            return new JObject { ["ok"] = true, ["pending"] = pending };
        }
        ...
        private static string NormalizeAppIcon(string value, string customPath)
        {
            ...
            if (string.Equals(safe, "replaykit.ico", StringComparison.OrdinalIgnoreCase)) return "default";
        ...
        // sound/media device setup class; one NNNN subkey per installed audio driver. mirrors VbCable.IsVbcableInstalled in the setup exe -- the settings dock uses this to show Install vs Uninstall on the Share Preview audio row.
        private const string MediaClassKey = @"SYSTEM\CurrentControlSet\Control\Class\{4d36e96c-e325-11ce-bfc1-08002be10318}";

        public static bool IsShareAudioDriverInstalled()
        {
            try
            {
        ...
        public static void SyncClipDirToRuntimeConfig(JObject settings)
        {
            try
            {
                if (Server.State.Config == null) return;
                string clipDir = settings?["clipDir"]?.Value<string>();
                if (string.IsNullOrWhiteSpace(clipDir)) return;
                if (string.Equals(Server.State.Config["clipDir"]?.Value<string>(), clipDir, StringComparison.Ordinal)) return;
                Server.State.Config["clipDir"] = clipDir;
            }
            catch (Exception ex) { Log.Write("SyncClipDirToRuntimeConfig: " + ex.Message); }
        }
        ...
        // removes the scene-item reference, not the source. used to drop duplicate top-level overlay items -- the
        // source stays alive because "Group" still references it.
        private static JObject RemoveSceneItem(string sceneName, JObject item)
        {
            if (item == null) return new JObject { ["ok"] = true, ["skipped"] = true };
            var r = ObsWebSocket.InvokeRequest("RemoveSceneItem", new JObject
            {
                ["sceneName"] = sceneName,
                ["sceneItemId"] = item["sceneItemId"]?.Value<int>() ?? 0,
            }, 3000);
        ...
        // the one scene item that carries the whole overlay for a style -- bongo is a single source, the input overlay is the wasd+mouse group.
        private static string OverlayItemNameForStyle(string style)
        {
            if (style == "bongo_cat") return "Bongo Cat Overlay";
            if (style == "input_overlay") return "Group";
            return null;
        }

        private static JObject GetCanvasSizeLive()
        {
            double w = 0.0, h = 0.0;
            var r = ObsWebSocket.InvokeRequest("GetVideoSettings", null, 3000);
            if (r.Ok)
            {
                w = GetDoubleValue(r.Data, "baseWidth", 0.0);
                h = GetDoubleValue(r.Data, "baseHeight", 0.0);
            }
            if (w <= 0.0 || h <= 0.0) { w = 1920.0; h = 1080.0; }
            return new JObject { ["width"] = w, ["height"] = h };
        }

        // resolves the live overlay item plus the scene it sits in, or nulls when obs/the item isnt there. style comes back so callers can say which overlay the rect actually belongs to.
        private static JObject ResolveOverlayItemLive(out string sceneName, out string style)
        {
            sceneName = null;
            style = Normalize(ReadSettings())["overlayStyle"]?.Value<string>() ?? "";
            string itemName = OverlayItemNameForStyle(style);
            if (itemName == null) return null;
            var scene = GetSceneName();
            if (scene["ok"]?.Value<bool>() != true) return null;
            sceneName = scene["name"]?.Value<string>();
            return FindSceneItem(GetSceneItems(sceneName)["items"] as JArray, itemName);
        }

        // the overlay's live rect as canvas fractions. the in-page projector seeds and polls this so it mirrors wherever the item actually sits, including drags the user did in obs itself.
        public static JObject GetOverlayPositionLive()
        {
            // the canvas size rides along even when the item cannot be resolved (overlay off, scene not built yet) so the fake projector still matches the real ones aspect and resolution
            JObject miss;
            try
            {
                var c = GetCanvasSizeLive();
                miss = new JObject { ["ok"] = true, ["available"] = false, ["canvasWidth"] = c["width"], ["canvasHeight"] = c["height"] };
            }
            catch { miss = new JObject { ["ok"] = true, ["available"] = false }; }
            try
            {
                var item = ResolveOverlayItemLive(out string sceneName, out string style);
                if (item == null) return miss;
                var cur = GetSceneItemTransformLive(sceneName, item);
                var t = cur["transform"] as JObject;
                if (cur["ok"]?.Value<bool>() != true || t == null) return miss;
                var canvas = GetCanvasSizeLive();
                double cw = canvas["width"].Value<double>(), ch = canvas["height"].Value<double>();
                double sx = GetDoubleValue(t, "scaleX", 1.0), sy = GetDoubleValue(t, "scaleY", 1.0);
                double w = GetDoubleValue(t, "width", 0.0), h = GetDoubleValue(t, "height", 0.0);
                if (w <= 0.0) w = GetDoubleValue(t, "sourceWidth", 0.0) * Math.Abs(sx);
                if (h <= 0.0) h = GetDoubleValue(t, "sourceHeight", 0.0) * Math.Abs(sy);
                double x = GetDoubleValue(t, "positionX", 0.0), y = GetDoubleValue(t, "positionY", 0.0);
                // a mirrored item is anchored on its right edge, so walk back to the visible top-left the preview draws from
                if (sx < 0.0) x -= w;
                if (sy < 0.0) y -= h;
                return new JObject
                {
                    ["ok"] = true, ["available"] = true,
                    ["x"] = cw > 0 ? x / cw : 0.0, ["y"] = ch > 0 ? y / ch : 0.0,
                    ["w"] = cw > 0 ? w / cw : 0.0, ["h"] = ch > 0 ? h / ch : 0.0,
                    ["canvasWidth"] = cw, ["canvasHeight"] = ch,
                    // which overlay this rect belongs to -- the preview ignores it while the style dropdown shows a different one, since that item is not in the scene until apply
                    ["style"] = style,
                    // the saved overlayScale this rect was produced at -- the preview divides by it so an unapplied size edit can grow the box from the real one
                    ["scalePct"] = OverlayScaleFactor(Normalize(ReadSettings())) * 100.0,
                };
            }
            catch (Exception ex) { Log.Write("GetOverlayPositionLive: " + ex.Message); return miss; }
        }
        ...
        // moves a freshly computed overlay transform onto the spot the user dragged it to, if there is one for this style. runs before
        // ApplyOverlayFlipH so the mirror shift still lands on top of it, and is what makes a drag survive the scene rewrite + restart
        // that every overlay-style change goes through -- a live SetSceneItemTransform alone would be thrown away by that reload.
        private static void ApplyOverlayPositionOverride(JObject transform, JObject preset, JObject settings, string style)
        {
            if (!(settings?["overlayPosition"] is JObject map)) return;
            if (!(map[style] is JObject at)) return;
            var content = OverlayContentRect(preset);
            double cw = content["canvasWidth"].Value<double>(), ch = content["canvasHeight"].Value<double>();
            if (cw <= 0.0 || ch <= 0.0) return;
            transform["positionX"] = at["x"].Value<double>() * cw;
            transform["positionY"] = at["y"].Value<double>() * ch;
        }
        ...
        private static JObject InputOverlayGroupTransform(JObject preset, JObject settings = null)
        {
            ...
            ApplyOverlayPositionOverride(transform, preset, settings, "input_overlay");
        ...
        private static JObject BongoTransform(JObject preset, JObject settings = null)
        {
            ...
            ApplyOverlayPositionOverride(transform, preset, settings, "bongo_cat");
        ...
        private static JObject ApplyOverlayLive(JObject settings, JObject preset, bool recreateBongo = true)
        {
            ...
            // SetOverlaySceneFile builds WASD/Mouse Overlay as members of "Group". GetSceneItemList only returns top-level
            // items, so a prior ApplyOverlayLive that ran EnsureInputSceneItem on them saw them as missing and its
            // CreateInput->CreateSceneItem fallback added a duplicate top-level copy every apply ("two Mouse Overlays").
            // If the Group is present, any top-level WASD/Mouse item is a stray -- drop it (the source survives).
            if (FindSceneItem(items, "Group") != null)
            {
                foreach (var stray in items.OfType<JObject>()
                             .Where(i => { var n = i["sourceName"]?.Value<string>(); return n == "WASD Overlay" || n == "Mouse Overlay"; })
                             .ToList())
                {
                    var rm = RemoveSceneItem(sceneName, stray);
                    if (rm["ok"]?.Value<bool>() != true) warnings.Add("Could not remove a duplicate " + stray["sourceName"]?.Value<string>() + " item: " + rm["message"]);
                }
                var refreshed = GetSceneItems(sceneName);
                if (refreshed["ok"]?.Value<bool>() == true) items = refreshed["items"] as JArray ?? items;
            }
            ...
            if (overlayStyle == "input_overlay")
            {
                ...
                else
                {
                    ...
                    // with a "Group" the two overlays are its members -- only refresh the source settings; adding them as
                    // standalone scene items is exactly the duplicate-stacking bug. no Group (legacy scene) -> ensure them
                    // as top-level items and position each one.
                    bool grouped = group != null;
                    foreach (var entry in entries)
                    {
                        ...
                        if (grouped)
                        {
                            var s = ObsWebSocket.InvokeRequest("SetInputSettings", new JObject { ["inputName"] = entry.name, ["inputSettings"] = io, ["overlay"] = true }, 3000);
                            if (!s.Ok) warnings.Add("Could not update " + entry.name + ": " + s.Message);
                        }
                        else
                        {
                            var created = EnsureInputSceneItem(sceneName, entry.name, "input-overlay", io, true);
        ...
        private static JObject ApplyLiveSettings(JObject settings, bool restartObs = false, bool applyOverlay = true, bool recreateBongo = true, bool applyMotionBlur = true, bool applyRuntimeOutputs = true, bool applyVideoSettings = true, bool applyReplayBufferOutput = true, bool applyMic = false, bool applyAudioVolumes = false)
        {
            ...
            // always pin obs's recording folder to the RESOLVED clip dir (custom or the ~/Videos default) -- the helper's
            // clip watcher/scanner uses that same resolved dir, so if obs's own RecFilePath ever drifts (a reset profile,
            // an obs update) clips land where nothing is looking for them.
        ...
        private static JObject ThemeMenuColors(JObject settings)
        {
            try
            {
                ...
                return new JObject
                {
                    ...
                    // the dock --grey5 / --grey3 pair, so the title-bar bell can paint the same hover surface and open
                    // border the clips sort trigger does under whatever theme is active. same mix Themes uses for --grey5.
                    ["surface"] = Themes.Mix(t.Panel, t.Field, 0.45),
                    ["border"] = t.Border,
                    // dock button parity for the plugin confirm dialog: panel = card ground, field = --button_bg,
                    // borderStrong = --button_border_hover. fieldHover is a lightness bump on field (the dock's grey4->grey3
                    // step) rather than t.Border, which on some themes equals t.Field and would give no visible hover.
                    ["panel"] = t.Panel,
                    ["field"] = t.Field,
                    ["fieldHover"] = Themes.Shift(t.Field, 0.09),
                    ["borderStrong"] = t.BorderStrong,
                    ["danger"] = t.Danger,
                    // --grey8, the ground the dock tooltips sit on, so the bells tip matches the sort triggers
                    ["tip"] = Themes.Shift(t.Bg, t.Dark ? -0.28 : -0.06),
                };
            }
        ...
        // self-heal: obs's recording folder (SimpleOutput.FilePath / AdvOut.RecFilePath) can drift off the RESOLVED clip
        // dir (custom, or the ~/Videos default) -- a reset profile, an obs update, a mid-session force-kill -- and then
        // new replay clips land where nothing is looking. runs deferred (obs websocket needs a moment) and only writes
        ...
        public static void EnsureObsRecordingFolderAtStartup()
        {
            Task.Run(() =>
            {
                try
                {
                    ...
                    // heals installs whose helper_config predates the folder being settable in Settings, so the clip listing stops walking the default folder
                    SyncClipDirToRuntimeConfig(new JObject { ["clipDir"] = resolved });
