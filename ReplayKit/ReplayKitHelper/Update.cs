...
namespace ReplayKitHelper
{
    ...
    internal static class Update
    {
        ...
        // sha256sum-style sidecar for the installer, one "<hash>  OBSReplayKit.exe" line. named after the file it covers, which is the only other asset on a release.
        private const string ChecksumsAsset = "OBSReplayKit.exe.sha256";
        private const string ReleasePage = "https://github.com/7Lzz/OBS-ReplayKit/releases/latest";

        // floor to reject a broken release before it closes obs -- a framework-dependent slip or a payload-less exe lands far under this. mirrors the same guard build.bat applies at the producing end.
        ...
        public static ReleaseInfo GetLatestRelease(bool manual = false)
        {
            ...
            lock (ReleaseCacheGate)
            {
                _releaseCache = fetched;
                _releaseCacheAtUtc = DateTime.UtcNow;
            }
            return fetched;
        }
        ...
        public static JObject GetStartupUpdateStatus()
        {
            ...
            // the first-run wizard reads this so it can stand aside until the update is done
            lock (Server.State.UpdateCheckLock) Server.State.StartupUpdatePending = status["prompt"]?.Value<bool>() ?? false;
        ...
        // opens the update window as its own detached process (OBSReplayKit.exe --update-window, handled in
        // UpdatePromptWindow.cs). it has to outlive this helper: an update closes obs, the helper follows obs down,
        // and the installer only copies files once the helper pid is gone -- a window hosted in-process would vanish
        // mid-install. SpawnDetached breaks it out of the helpers kill-on-close job so that chain never reaches it.
        // same window whether the startup check or the Settings button opened it.
        public static JObject OpenUpdatePromptWindow(string version)
        {
            // one window is enough -- bring the existing one forward rather than stacking another.
            try
            {
                if (Native.WindowWithTitleExists("ReplayKit Update"))
                {
                    Native.FocusWindowByExactTitle("ReplayKit Update");
                    WriteUpdateDebug("update window already open; focused it instead of opening another");
            ...
            var argList = new List<string> { "--update-window", "-Port", port.ToString() };
            if (!string.IsNullOrWhiteSpace(version)) { argList.Add("-Version"); argList.Add(version); }
            // the window process has no settings loaded -- hand it the active theme's window colour so the pre-paint
            // frame (before the page renders) matches the Appearance tab; the border is the dwm system default
            try
            {
                var theme = Themes.Resolve(ReplaykitSettings.Normalize(ReplaykitSettings.ReadSettings()));
                if (!string.IsNullOrWhiteSpace(theme.Bg)) { argList.Add("-BgColor"); argList.Add(theme.Bg); }
            }
            catch (Exception ex) { WriteUpdateDebug("update window theme resolve failed: " + ex.Message); }
            string cmdLine = ProcessArgs.Quote(exe) + " " + ProcessArgs.Join(argList.ToArray());
            try
            {
                int pid = Native.SpawnDetached(cmdLine, Constants.REPLAYKIT_TEMP_ROOT);
                if (pid <= 0) throw new InvalidOperationException("SpawnDetached returned 0 for the update window.");
                WriteUpdateDebug("update window spawned detached pid=" + pid + " port=" + port);
                return new JObject { ["ok"] = true, ["processId"] = pid };
            }
            catch (Exception ex)
            {
                WriteUpdateDebug("update window spawn failed: " + ex.Message);
        ...
        // live progress for the updater window. it installs on its own now, so the bar has to be real numbers rather
        // than a timer -- stage plus byte counts plus the speed derived from them, exactly what the window renders.
        private static readonly object ProgressGate = new object();
        private static string _progressStage = "";
        private static long _progressDone, _progressTotal;
        private static DateTime _progressStartUtc = DateTime.UtcNow;
        ...
        private static void SetProgressBytes(long done, long total)
        {
            lock (ProgressGate) { _progressDone = done; _progressTotal = total; }
        }

        public static JObject GetProgress()
        {
            lock (ProgressGate)
            {
                double seconds = Math.Max(0.001, (DateTime.UtcNow - _progressStartUtc).TotalSeconds);
                return new JObject
                {
                    ["ok"] = true,
                    ...
                    ["bytesPerSecond"] = _progressDone > 0 ? _progressDone / seconds : 0.0,
                };
            }
        }

        private static void SaveUrlFile(string url, string path, bool report = false)
        {
            ...
            try
            {
                ...
                using (var response = Http.GetAsync(uri, HttpCompletionOption.ResponseHeadersRead, timeout.Token).GetAwaiter().GetResult())
                {
                    ...
                    long expected = response.Content.Headers.ContentLength ?? 0L;
                    if (report) SetProgressBytes(0, expected);
                    ...
                    using (var fileStream = new FileStream(temp, FileMode.CreateNew, FileAccess.Write, FileShare.None))
                    {
                        ...
                        while ((count = stream.ReadAsync(buffer, 0, buffer.Length, timeout.Token).GetAwaiter().GetResult()) > 0)
                        {
                            ...
                            if (report) SetProgressBytes(total, expected);
                        }
        ...
        // "<hash>  <filename>" per line, standard sha256sum shape (a leading "*" before the filename, for binary-mode tools, is tolerated but never written by build.bat). parsed as a table rather than a bare hash so the sidecar stays readable by ordinary sha256sum -c.
        ...
        // compares the downloaded installer against the hash parsed out of its .sha256 sidecar.
        ...
        public static JObject ApplyUpdate()
        {
            ...
            try
            {
                ...
                SaveUrlFile(latest.InstallerUrl, installerPath, true);
