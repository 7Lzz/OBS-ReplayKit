...
function Get-ReplayKitVersion([string]$versionInfoPath) {
    # ReplayKitSetup/VersionInfo.cs is the single source of truth for release version (see build.bat); read it instead of hardcoding a copy here.
...
$versionInfoPath = Join-Path $PSScriptRoot '..\..\ReplayKitSetup\VersionInfo.cs'
$versionCsPath = Join-Path $PSScriptRoot 'AssemblyVersion.Generated.cs'

$launcherInputs = @($srcPath, $csc, $sma, $versionInfoPath)
...
$version = Get-ReplayKitVersion $versionInfoPath
Write-Host ("version: " + $version)
@"
// generated by build_launcher.ps1 from ReplayKitSetup/VersionInfo.cs -- do not edit by hand, changes are overwritten on the next build.
using System.Reflection;
[assembly: AssemblyVersion("$version")]
[assembly: AssemblyFileVersion("$version")]
"@ | Set-Content -LiteralPath $versionCsPath -Encoding UTF8
