...
function Get-ReplayKitVersion([string]$initPyPath) {
    # obs_replaykit/__init__.py is the single source of truth for release version (see build.bat); read it instead of hardcoding a copy here.
    if (-not (Test-Path -LiteralPath $initPyPath)) {
        throw "Missing version source: $initPyPath"
    }
    $match = Select-String -LiteralPath $initPyPath -Pattern '__version__\s*=\s*"([^"]+)"' | Select-Object -First 1
    if (-not $match) {
...
$initPyPath = Join-Path $PSScriptRoot '..\..\obs_replaykit\__init__.py'
$versionCsPath = Join-Path $PSScriptRoot 'AssemblyVersion.Generated.cs'

$launcherInputs = @($srcPath, $csc, $sma, $initPyPath)
...
$version = Get-ReplayKitVersion $initPyPath
Write-Host ("version: " + $version)
@"
// generated by build_launcher.ps1 from obs_replaykit/__init__.py -- do not edit by hand, changes are overwritten on the next build.
