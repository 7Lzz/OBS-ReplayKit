...
function Start-CompressOverwriteFile($selected, [string]$mode = 'smaller') {
    ...
    $tempPath = Join-Path $env:TEMP ("streamable_compress_" + $requestId + $ext)
    ...
    # two encode profiles. "fast" uses libx264 fast crf 24 -- encodes at multi-x realtime on modern cpus and produces files ~50-70% of source, h264 plays everywhere. "smaller" uses libx265 medium crf 28 -- encodes at sub-realtime but cuts file size ~half vs libx264 at indistinguishable quality (hvc1 tag for Chromium/CEF playback support). progress is captured by piping ffmpegs -progress to stdout (rather than to a file) and reading it line-by-line from the worker. -progress <file> works but windows file caching can delay reads relative to ffmpegs writes; the pipe path is bounded by ffmpegs fflush() and is the standard fix.
    $workerScript = @'
param(
    [string]$Ffmpeg,
    [string]$InputPath,
...
# half-cores per encode for sw codecs. two parallel libx265/libsvtav1 jobs split the cpu cleanly with this -- one job grabbing every core context-switches itself silly when a second job lands.
...
# hidden marker embedded in the mp4s udta:comment atom so the helper can later
# identify files weve already compressed without a sidecar db. format:
# obs-replaykit_compress_v2_<fast|slow>:<unix_ms>:<pre_compress_bytes>.
# "fast" and "slow" map to the fast and smaller modes; the actual encoder is
# picked from caps below.
...
switch ($pickedEncoder) {
    'av1_nvenc' {
        # nvidia av1 encoder (ada / blackwell, rtx 40-/50-series). ~30% more efficient than hevc_nvenc; cef / chromium / discord / win11 native players all decode av1 fine. qp is on the 0-255 av1 scale here (not 0-51 like hevc/h.264). empirical sweep on a hevc_nvenc qp24 1080p60 source: qp 120 -> 86% of source (light, near-imperceptible loss) qp 130 -> 47% of source (balanced) qp 130 + spatial-aq + lookahead -> 40% of source (better quality and smaller, per probe) qp 150 -> 28% (aggressive, noticeable artifacts) settled on qp 130 with spatial-aq + lookahead for the "compress meaningfully without obvious quality loss" target. spatial-aq is safe here becuase rc=constqp (it just modulates the fixed qp per region without raising the bitrate ceiling). the old vbr+cq+b:v 0 + spatial-aq combination was the one that ballooned files -- diffrent rate control entirely. no -profile flag: av1_nvenc has no `profile` avoption.
        $argv = $commonHead + @(
            '-c:v','av1_nvenc',
            '-preset','p4',
            '-tune','hq',
    ...
    'h264_nvenc' {
        # H.264 needs ~30% more bits than HEVC for the same quality, so the QP
        # target sits a couple notches above HEVC.
        $argv = $commonHead + @(
            '-c:v','h264_nvenc',
            '-preset','p4',
            '-tune','hq',
    ...
    'libsvtav1' {
        # intel svt-av1, the modern "actualy small and fast" cpu encoder. preset 6 is the sweet spot: comparable speed to libx265 medium, files ~30% smaller at matched quality. crf 28 (slightly under the previous crf 30) keeps an extra notch of perceptual quality. lp=$swpools caps logical processors so two parallel jobs dont fight for cores.
        $argv = $commonHead + @(
            '-c:v','libsvtav1',
            '-preset','6',
            '-crf','28',
            '-svtav1-params',"lp=$swPools:tune=0",
            '-pix_fmt','yuv420p'
        ) + $commonTail
    }
    'libx265' {
...
# size guard. compression is supposed to shrink files; if it didnt, dont replace the original. this catches: very short clips where mp4 header + first idr + aac priming overhead exceeds whatever the encode saved sources already encoded more aggressively than our target (e.g. obs "performance" preset at cqp 28 vs our qp 30 fast mode -- still smaller but barely; the size guard surfaces edge cases) any future encoder misconfiguration we havent anticipated the clip stays untouched, the marker is not written, and the next /clips poll shows the file in its original state. the user sees a clean "already at minimum size" message.
$outBytes = 0
...
try {
    # same-volume move-item is an atomic rename. across volumes it falls back to copy-then-delete which can leave the destination half-written on crash. detect the volume mismatch and route thru a sidecar on the sources volume so the final replace is always atomic. the sidecar is named with the _streamable_ prefix that the clip listing filters out.
    ...
    if ($sourceVol.ToLowerInvariant() -eq $tempVol.ToLowerInvariant()) {
        ...
        $sideTemp = Join-Path ([System.IO.Path]::GetDirectoryName($sourcePath)) ("_streamable_finalize_" + [Guid]::NewGuid().ToString('N') + $extOut)
...
if ($dbPath) {
    ...
    $haveLock = $false
    try {
        $mutex = New-Object System.Threading.Mutex($false, 'Global\OBSReplayKit_ClipsDb')
        $haveLock = $mutex.WaitOne(5000)
        if ($haveLock) {
