...
function Copy-WithVSS([string]$src, [string]$dst) {
    ...
    try {
        ...
        try {
            ...
            $junction = Join-Path $env:TEMP ('vss_' + [Guid]::NewGuid().ToString('N'))
            try {
...
function Read-StreamableCookiesFromSource($source) {
    ...
    $tempDir = Join-Path $env:TEMP ("strmbl_helper_" + [Guid]::NewGuid().ToString('N'))
    ...
    try {
        ...
        if ($msg -match 'Both direct copy and VSS failed' -and -not (Test-IsAdmin)) {
            $script:LastReadError = 'locked file, not admin'
        } else {
            $script:LastReadError = $msg
        }
...
function Find-WorkingStreamableSession([switch]$ObsOnly) {
    ...
    foreach ($s in $sources) {
        ...
        if ($cachedMiss) {
            ...
            $script:LastReadError = $reason
            if (-not ($reason -and $reason -match 'not admin')) { $allLockedNoAdmin = $false }
            continue
        }
        $cookies = Read-StreamableCookiesFromSource $s
        if (-not $cookies -or $cookies.Count -eq 0) {
            ...
            $lastErr = $script:LastReadError
            $missReason = if ($lastErr) { [string]$lastErr } else { 'no streamable cookies' }
            Set-CachedCookieMiss $s $sourceSignature $missReason
            if (-not ($lastErr -and $lastErr -match 'not admin')) { $allLockedNoAdmin = $false }
            continue
        }
