...
public static class StreamableNative {
    ...
    // save a 480x270 jpeg thumbnail for the given video file. the windows shell extracts the frame using whatever decoder is registered (directshow, media foundation) so mp4/mov/mkv all work without us shipping ffmpeg.
    ...
    // foreground-window apis used by focuswindow. the bare setforegroundwindow call is subject to windows anti-focus stealing protection -- a process can usually only set foreground when its already the foreground process, was started by the foreground process, or just recieved user input. our helper
    // as obs is currently the foreground app. to make this robust regardless of which obs window owns focus right now, we use the attachthreadinput trick: temporarily share the input queue with the current foreground thread, call setforegroundwindow, detach. universally compatible across windows versions.
    ...
    // has the process behind this handle exited implemented as a
    // getexitcodeprocess poll (handle doesnt need synchronize access). the
    // timeoutms parameter is currently ignored -- callers that need to wait
    // should do start-sleep between calls.
    ...
    public static bool FocusWindow(string needle) {
        ...
        EnumWindows(delegate(IntPtr hWnd, IntPtr lParam) {
            ...
            FocusHwnd(hWnd);
            focused = true;
            return false;   // stop enumerating, we're done
        }, IntPtr.Zero);
        return focused;
    }
