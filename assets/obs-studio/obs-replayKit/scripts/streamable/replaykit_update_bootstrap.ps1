# standalone update-prompt launcher. spawned by streamable_upload.lua right after the helper, so the
# check works even when obs is minimized to the system tray and the cef dock is suspended (which is
# what was killing the dock-side window.open at startup).
#
# flow: wait for the helper to bind its port -> call /update/startup-check -> if an update is
# pending, open update_prompt.html in a borderless msedge --app window. winforms messagebox is the
# fallback when edge isnt installed. every step is appended to %temp%\replaykit_update_debug.log so
...
$script:DebugLogPath = Join-Path $env:TEMP 'replaykit_update_debug.log'
...
# step 1: ask the helper if a prompt is needed. /update/startup-check sets the once-per-launch flag
# on the helper, so the dock wont also try to open its own popup afterwards. retry a few times in
# case the helper takes longer than $waitseconds to bind the port on slow boxes.
$resp = $null
...
# step 2: prefer a chromium browser in --app mode -- it renders the existing update_prompt.html
# which is already styled to match obs, and the page handles /update/apply + /update/later itself.
# msedge, then chrome, then brave, then vivaldi -- first one we find wins.
$browserCandidates = @(
...
if ($browserPath) {
    ...
    $profileDir = Join-Path $env:TEMP ("ReplayKitUpdatePopupProfile-$browserName")
    try { [void](New-Item -ItemType Directory -Path $profileDir -Force -ErrorAction SilentlyContinue) } catch {}

    # center on the primary monitor. winforms is the cheapest way to get screen bounds from ps; if
    # the load fails (Server Core etc.) we fall back to a sensible fixed offset rather than abort.
    $winWidth = 560
    $winHeight = 460
    $posX = 200
    $posY = 200
...
# step 3: winforms messagebox fallback. ugly compared to the html popup but always available on
# windows, so the user never silently misses the update.
try {
