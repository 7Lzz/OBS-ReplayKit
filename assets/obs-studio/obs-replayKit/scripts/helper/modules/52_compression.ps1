...
function Get-HelperCapabilities([switch]$Refresh) {
    ...
    # CapabilitiesLock nests outside ConfigLock/ClipsMetaLock (Load-Config below acquires those internally), never the reverse, so this cant deadlock -- double-checked since a concurrent cache-miss caller may have already refreshed by the time we get the lock.
    [System.Threading.Monitor]::Enter($script:State.CapabilitiesLock)
    try {
    if (-not $Refresh -and $script:State.Capabilities -and
        (($now - $script:State.CapabilitiesAt).TotalMinutes -lt 10)) {
        return $script:State.Capabilities
    ...
    $fast = 'libx264'
    foreach ($cand in @('hevc_nvenc','hevc_amf','hevc_qsv','h264_nvenc','h264_amf','h264_qsv','libx264')) {
        if ($encoders[$cand]) { $fast = $cand; break }
    }
    $smaller = 'libx264'
    foreach ($cand in @('libx265','libx264')) {
    ...
    $caps = @{
        ...
        logging       = [bool]$script:State.LogEnabled
    ...
    return $caps
    } finally { [System.Threading.Monitor]::Exit($script:State.CapabilitiesLock) }
}
...
function Start-CompressedStreamableUpload($selected) {
    ...
    $uploadLogFile = if ($script:State.LogEnabled) { $script:UPLOAD_LOG_PATH } else { '' }
...
'@
    ...
    $psi.EnvironmentVariables['S_LOGGING_ENABLED'] = if ($script:State.LogEnabled) { '1' } else { '0' }
