# bootstrap: open the listener, set up the watcher runspace pool, build the connection pools initialsessionstate, accept loop -- request handling is multithreaded, each accepted connection dispatched to a runspace pulled from $script:ConnectionPool instead of handled inline, so a slow /file range read no longer blocks unrelated requests. a brand new runspace normally has none of this sessions dot-sourced functions, which is what the iss built below sidesteps (registering every module function + the read-only constants + $script:State) instead of just staying single-threaded; every piece of state more than one request could touch concurrently is guarded by a Monitor lock on $script:State (see the comment on it in 00_state.ps1 for the full list and locking conventions).
Load-Config
Clear-ReplayKitLogsAtStartup
...
Write-Log "ReplayKit helper listening on http://$script:HOST_ADDR`:$port"
...
if ($script:ParentPid -gt 0) {
    try {
        $script:ParentHandle = [ReplayKitNative]::OpenParentForSync([uint32]$script:ParentPid)
...
try { Start-ReplayKitDiscordProjectorAtStartup } catch { Write-Log "warn: Discord projector startup threw: $($_.Exception.Message)" }
...
try {
    $script:JobHandle = [ReplayKitNative]::CreateKillOnCloseJob()
...
# connection pool: every accepted socket below is handed to a runspace pulled from this instead of being handled inline on the accept-loop thread -- a brand new runspace normally has none of this sessions dot-sourced functions (the reason this server was single-threaded to begin with), so an initialsessionstate built from the functions and constants below is what makes a pooled runspace able to call Dispatch-Request/Handle-Connection and every route handler without re-dot-sourcing all 21 files per connection. $newFunctionNames is every function defined since local_helper_server.ps1 snapshotted Function:\ right before the dot-source loop. CreateDefault(), not Create() -- Create() omits built-in cmdlets (ConvertFrom-Json, Get-Process, etc) that handlers call constantly, which would fail in a confusingly similar way to a real registration bug.
$newFunctionNames = (Get-ChildItem Function:\).Name | Where-Object { $_ -notin $script:PreloadFunctionNames }
$iss = [System.Management.Automation.Runspaces.InitialSessionState]::CreateDefault()
# a runspace pool built from a plain CreateDefault() ISS defaults its runspaces to MTA -- fine for nearly every handler here, but several do real OLE/COM work (Clipboard.SetFileDropList for /clipboard, Shell.Application and WScript.Shell) that hard-requires STA and throws otherwise; this server was single-threaded until recently, running on powershell.exes own STA-by-default main thread, so none of those call sites had ever needed to think about apartment state until moving real work onto pooled runspaces exposed it -- confirmed empirically that CreateDefault()s runspaces come up MTA and that setting this before CreateRunspacePool below is what actually changes it (2026-08-08).
$iss.ApartmentState = [System.Threading.ApartmentState]::STA
foreach ($fname in $newFunctionNames) {
    $fn = Get-Item -LiteralPath "Function:\$fname" -ErrorAction SilentlyContinue
    if (-not $fn) { continue }
    $iss.Commands.Add([System.Management.Automation.Runspaces.SessionStateFunctionEntry]::new($fname, $fn.Definition))
}
# read-only constants (PooledConstantNames, built up in 00_state.ps1 + a few other modules next to where each constant is declared) plus $script:State itself, seeded once -- enough becuase its fields mutate in place, the object itself never reassigned, so every pooled runspace ends up sharing the exact same synchronized hashtable as the main thread.
foreach ($vname in $script:PooledConstantNames) {
    $v = Get-Variable -Name $vname -Scope Script -ErrorAction SilentlyContinue
    if ($v) { $iss.Variables.Add([System.Management.Automation.Runspaces.SessionStateVariableEntry]::new($vname, $v.Value, '')) }
}
$iss.Variables.Add([System.Management.Automation.Runspaces.SessionStateVariableEntry]::new('State', $script:State, ''))

# bounded pool pulling from that iss, not one thread per connection -- the same shape Pode (the established powershell web framework) uses. min 2 so the first couple of real concurrent connections dont pay runspace-creation latency; max sized off core count since each runspace is a real (~10-20mb) resource and this is a loopback server for a handful of dock uis, not an internet-facing one -- MAX_PREVIEW_STREAM alone caps concurrent video streams at 2, so this leaves plenty of headroom over anything actually expected to run at once.
$script:MAX_CONNECTION_THREADS = [Math]::Max(6, [Environment]::ProcessorCount)
$script:ConnectionPool = [runspacefactory]::CreateRunspacePool(2, $script:MAX_CONNECTION_THREADS, $iss, $Host)
$script:ConnectionPool.Open()
# {Ps;Async} pairs for connections thats been dispatched but not yet reaped -- see the sweep in the accept loop below; touched only by the main accept-loop thread, so it needs no lock of its own.
$script:PendingConnections = New-Object System.Collections.Generic.List[object]
...
# set the instant we confirm obs64 itself is gone, as opposed to exiting for an explicit /shutdown (script reload) where obs is normally still running -- the finally block only wipes the crash sentinel when this is true, so a reload restart cant prematurely clear a still-live sessions sentinel.
$script:ParentDied = $false
function Check-ParentAlive {
    ...
    if ($script:ParentHandle -ne [IntPtr]::Zero) {
        if ([ReplayKitNative]::ParentExited($script:ParentHandle, 0)) {
            Write-Log "Parent process handle signaled (terminated). Exiting."
            $script:ParentDied = $true
            return $false
        }
    }
    $now = [DateTime]::UtcNow
...
# obs writes a run_<uuid> file under %appdata%\obs-studio\.sentinel on launch and deletes every lingering one during its own applicationShutdownHandler on a clean exit -- a killed/crashed obs never reaches that code, so the leftover file is what makes the next launch show the "crash detected" prompt; clearing it ourselves the moment we know obs is actually gone (see $script:ParentDied above) heads that off regardless of how obs went down, including a raw task manager "end task" that our own restart flow never gets a chance to see.
function Clear-ObsCrashSentinel {
    $sentinelDir = Join-Path $env:APPDATA 'obs-studio\.sentinel'
    if (-not (Test-Path -LiteralPath $sentinelDir)) { return }
    Get-ChildItem -LiteralPath $sentinelDir -Filter 'run_*' -Force -ErrorAction SilentlyContinue |
        ForEach-Object {
            try {
                Remove-Item -LiteralPath $_.FullName -Force -ErrorAction Stop
                Write-Log "Clear-ObsCrashSentinel: removed $($_.Name)"
            } catch {
                Write-Log "Clear-ObsCrashSentinel: could not remove $($_.Name): $($_.Exception.Message)"
            }
        }
}
...
try {
    while (-not $script:State.Shutdown) {
        ...
        # reap connections whose pool runspace already finished, every iteration not just the idle branch below -- under sustained traffic Pending() is almost always true, so gating this on idle would let completed-but-undisposed entries pile up for the length of a busy stretch. the pool runspace itself is already returned to the pool the instant its script finishes, independent of this; this just surfaces/logs any unhandled exception and frees the wrapper [powershell] instance promptly instead of waiting on gc.
        for ($i = $script:PendingConnections.Count - 1; $i -ge 0; $i--) {
            $entry = $script:PendingConnections[$i]
            if ($entry.Async.IsCompleted) {
                try { [void]$entry.Ps.EndInvoke($entry.Async) }
                catch { Write-Log "connection worker error: $($_.Exception.Message)" }
                if ($entry.Ps.HadErrors) {
                    foreach ($e in $entry.Ps.Streams.Error) { Write-Log "connection worker stream error: $($e.Exception.Message)" }
                }
                try { $entry.Ps.Dispose() } catch {}
                $script:PendingConnections.RemoveAt($i)
            }
        }
        if (-not $listener.Pending()) {
            ...
            if ($script:ParentHandle -ne [IntPtr]::Zero -and
                [ReplayKitNative]::ParentExited($script:ParentHandle, 0)) {
                Write-Log "Parent terminated during idle (GetExitCodeProcess). Exiting."
                $script:ParentDied = $true
                break
            }
            try { Invoke-ReplayKitDiscordProjectorKeepAlive } catch { Write-Log "warn: Discord projector keep-alive threw: $($_.Exception.Message)" }
            Start-Sleep -Milliseconds 50
            continue
        }
        $client = $listener.AcceptTcpClient()
        # hand the connection to a pooled runspace instead of handling it inline -- this is the one line that makes the server multithreaded; everything else in this file exists to support it (the iss above) or clean up after it (the reap loop above, the Close() skip below).
        $dispatchPs = [powershell]::Create()
        $dispatchPs.RunspacePool = $script:ConnectionPool
        $null = $dispatchPs.AddScript({
            param($Client)
            try { Handle-Connection $Client }
            catch { Write-Log "handler error: $($_.Exception.Message)" }
        })
        $null = $dispatchPs.AddParameter('Client', $client)
        try {
            $async = $dispatchPs.BeginInvoke()
            $script:PendingConnections.Add([pscustomobject]@{ Ps = $dispatchPs; Async = $async })
        } catch {
            Write-Log "dispatch error: $($_.Exception.Message)"
            try { $dispatchPs.Dispose() } catch {}
    ...
    if ($script:ParentDied) { try { Clear-ObsCrashSentinel } catch {} }
    ...
    # same reasoning for $script:ConnectionPool -- skip close() there too; connections are far shorter-lived than an encode (a kept-alive /file/ stream is the longest-running case, bounded by the poll loop in 80_connection.ps1), so this is even lower-risk than the watcherpool case.
    ...
    if ($script:State.ClearStreamableOnExit) {
    ...
    if ($script:State.RestartAfterClean -and $script:State.RestartAfterClean.obsPath) {
        $obsPath = $script:State.RestartAfterClean.obsPath

        # same leftover-sentinel problem Clear-ObsCrashSentinel exists for -- were about to relaunch right after our own stop-process, which skips obss graceful-shutdown cleanup same as any other force-kill.
        try { Clear-ObsCrashSentinel } catch {}
