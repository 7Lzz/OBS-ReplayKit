...
function Invoke-StreamableBackgroundLogout {
    ...
    $respPath = Join-Path $script:SCRATCH_DIR ('streamable_logout_' + [Guid]::NewGuid().ToString('N') + '.txt')
    try {
...
function Invoke-StreamableMe([string]$cookieFileContent) {
    $jar = Join-Path $script:SCRATCH_DIR ('streamable_me_' + [Guid]::NewGuid().ToString('N') + '.txt')
    $respPath = Join-Path $script:SCRATCH_DIR ('streamable_me_resp_' + [Guid]::NewGuid().ToString('N') + '.txt')
    try {
